...
def get_target_plane_points(
    ...
    x = torch.linspace(
        -wx / 2.0, wx / 2.0, resolution[0] * resolution_factor, device=device
    )
    y = torch.linspace(
        -wy / 2.0, wy / 2.0, resolution[1] * resolution_factor, device=device
    )
...
def get_aperture_points(
    ...
    x = torch.linspace(
        -dimensions[0] / 2.0,
        dimensions[0] / 2.0,
        aperture_pattern.shape[-2],
        device=device,
    )
    y = torch.linspace(
        -dimensions[1] / 2.0,
        dimensions[1] / 2.0,
        aperture_pattern.shape[-1],
        device=device,
    )
    X, Y = torch.meshgrid(x, y, indexing="ij")
    X = X[aperture_pattern > 0.0]
    Y = Y[aperture_pattern > 0.0]
    X = X.reshape(-1, 1)
    Y = Y.reshape(-1, 1)
    Z = torch.ones_like(X) * z
    ...
    aperture_field = odak.learn.wave.generate_complex_field(
        aperture_pattern, aperture_phase
    )
    aperture_field = aperture_field[aperture_pattern > 0.0]
...
def main(
    ...
    aperture_pattern = odak.learn.tools.load_image(
        aperture_pattern_filename, normalizeby=255.0, torch_style=True
    ).to(device)[0]
    aperture_points, aperture_field = get_aperture_points(
        aperture_pattern, dimensions=[pixel_pitch, pixel_pitch], device=device
    )
    h = torch.zeros(
    ...
    for i in range(nx):
        for j in range(ny):
            shift = torch.tensor(
                [
                    [
                        pixel_pitch / nx * i - pixel_pitch / 2.0,
                        pixel_pitch / ny * j - pixel_pitch / 2.0,
                        0.0,
                    ]
                ],
                device=device,
