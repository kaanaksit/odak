...
def test(
    ...
    assert rotx.shape == torch.Size(
        [1, 3, 3]
    ), "Rotation matrix X shape should be [1, 3, 3]"
    assert roty.shape == torch.Size(
        [1, 3, 3]
    ), "Rotation matrix Y shape should be [1, 3, 3]"
    assert rotz.shape == torch.Size(
        [1, 3, 3]
    ...
    assert samples_zero.shape == torch.Size(
        [9, 3]
    ...
    assert torch.allclose(
        samples_zero, expected_positions, atol=1e-6
    ...
    assert samples_rot.shape == torch.Size(
        [4, 3]
