...
def test(
    output_directory="test_output", header="test/test_learn_raytracing_detector.py"
):
    odak.tools.check_directory(output_directory)
    device = torch.device("cpu")
    detector_size = torch.tensor([0.1, 0.1], device=device)
    detector_resolution = torch.tensor([100, 100], device=device)
    detector_center = torch.tensor([0.0, 0.0, 0.0], device=device)
    detector_tilt = torch.tensor([0.0, 0.0, 0.0], device=device)
    detector_colors = 1
    mesh_size = torch.tensor([0.1, 0.1], device=device)
    mesh_no = torch.tensor([20, 20], device=device)
    mesh_center = torch.tensor([0.0, 0.0, 0.1], device=device)
    ...
    start_points, _, _, _ = odak.learn.tools.grid_sample(
        no=ray_no, size=ray_size, center=ray_start
    )
    end_points, _, _, _ = odak.learn.tools.grid_sample(
        no=ray_no, size=ray_size, center=ray_end
    )
    ...
    target = (
        odak.learn.tools.load_image(
            "test/data/kaan.png", normalizeby=255.0, torch_style=True
        )[1]
        .unsqueeze(0)
        .to(device)
    )
    ...
    odak.learn.tools.save_image(
        "{}/ray_target.png".format(output_directory), target, cmin=0.0, cmax=1.0
    )
    ...
    for step in t:
        ...
        target_locations = torch.sum(
            1.0
            / mu
            / torch.sqrt(torch.tensor(2 * odak.pi))
            * torch.exp(-((distance_image - distance_min) ** 2) / 2.0 / mu**2),
            dim=0,
        )
        target_locations = (
            target_locations.reshape(1, 100, 100) / target_locations.max()
        )
    ...
    if visualize:
        ...
        for triangle_id in range(triangles.shape[0]):
            ray_diagram.add_triangle(
                triangles[triangle_id], row=1, column=1, color="orange"
            )
