...
def test_cvd_loss_custom_weights():
    ...
    total_loss, lc_loss, ci_loss = cvd_loss(
        image, simulated_image, alpha=10.0, beta=2.0
    )
...
def test_cvd_loss_extreme_weights():
    ...
    # Very high alpha
    total_loss_high_alpha, lc_loss_alpha, _ = cvd_loss(
        image, simulated_image, alpha=100.0, beta=1.0
    )

    # Very high beta
    total_loss_high_beta, _, ci_loss_beta = cvd_loss(
        image, simulated_image, alpha=1.0, beta=100.0
    )
    ...
    (
        100.0 * lc_loss_alpha.item() + 1.0 * ci_loss_beta.item()
    )  # approximate
    assert (
        abs(total_loss_high_alpha.item() - 100.0 * lc_loss_alpha.item())
        < 1.0 + ci_loss_beta.item()
    )
...
def test_cvd_loss_different_sigma_values():
    ...
    # Small sigma
    loss_small_sigma, _, _ = cvd_loss(image, simulated_image, ci_sigma=0.5)

    # Large sigma
    loss_large_sigma, _, _ = cvd_loss(image, simulated_image, ci_sigma=3.0)
