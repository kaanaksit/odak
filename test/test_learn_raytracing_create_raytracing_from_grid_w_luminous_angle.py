...
def test(directory="test_output"):
    ...
    rays_from_grid_w_luminous_angle = (
        odak.learn.raytracing.create_ray_from_grid_w_luminous_angle(
    ...
    propagated_rays = odak.learn.raytracing.propagate_ray(
        rays_from_grid_w_luminous_angle, distances
    )
    ...
    if visualize:
        ...
        ray_diagram.add_line(
            rays_from_grid_w_luminous_angle[:, 0], propagated_rays[:, 0]
        )
