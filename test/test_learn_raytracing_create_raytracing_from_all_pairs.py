...
def test(directory="test_output"):
    ...
    starting_points, _, _, _ = odak.learn.tools.grid_sample(
        no=[2, 2], size=[20.0, 20.0], center=[0.0, 0.0, 0.0]
    )
    ...
    rays_from_points_to_points = odak.learn.raytracing.create_ray_from_all_pairs(
    ...
    propagated_rays = odak.learn.raytracing.propagate_ray(
        rays_from_points_to_points, distances
    )
