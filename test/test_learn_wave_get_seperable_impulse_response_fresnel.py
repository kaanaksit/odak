    ...
    for wavelength_id, wavelength in enumerate(wavelengths):
        for dx_id, dx in enumerate(pixel_pitches):
            for distance_id in tqdm(range(len(distances))):
                ...
                _, kernel, kernel_x, kernel_y = (
                    odak.learn.wave.get_seperable_impulse_response_fresnel_kernel(
                        nu=resolution[0],
                        nv=resolution[1],
                        dx=dx,
                ...
                kernels[wavelength_id, distance_id, dx_id] = kernel.detach().clone()
                kernels_x[wavelength_id, distance_id, dx_id] = kernel_x.detach().clone()
                kernels_y[wavelength_id, distance_id, dx_id] = kernel_y.detach().clone()
