...
def test():
    try:
        ...
        odak.log.logger.warning(
            "{} -> ColorVideoVDP is missing, consider installing by visiting: https://github.com/gfxdisp/ColorVideoVDP".format(
                header
            )
        )
        assert True == True
...
def main(
    ...
    display_geometry = pycvvdp.vvdp_display_geometry(
        [image.shape[-2], image.shape[-1]], ppd=pixels_per_degree
    )
    display_photometry = pycvvdp.vvdp_display_photometry.load(
        display_standard, config_paths=display_config_paths
    )
