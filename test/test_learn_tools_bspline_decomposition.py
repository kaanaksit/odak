...
from odak.learn.tools import (
    decompose_wavelet_like,
    reconstruct_wavelet_like,
    save_image,
    load_image,
    save_torch_tensor,
)
...
def test():
    ...
    logger.info(
        "{} -> Wavelet-like Decomposition Test for Phase Holograms".format(header)
    )

    # Load the phase hologram
    loading_failed = True
    try:
        phase_img = load_image(
            "test/data/sample_direct_phase_hologram.png", normalizeby=255.0
        )
    ...
    total_data = (
        sum(len(c) for c in coefficients)
        + sum(r.numel() for r in residuals)
        + base.numel()
    )
    ...
    save_image(
        ...
        diff.unsqueeze(0),
        cmin=0,
        cmax=float(diff.max()) if diff.max() > 0 else 1e-10,
    )
    ...
    for scale_idx, residual in enumerate(residuals):
        save_image(
            os.path.join(
                output_dir, "wavelet_residual_scale_{}.png".format(scale_idx + 1)
            ),
    ...
    for scale_idx, coeffs in enumerate(coefficients):
        ...
        coef_norm = (coef_grid - coef_grid.min()) / (
            coef_grid.max() - coef_grid.min() + 1e-10
        )
        save_image(
            os.path.join(
                output_dir, "wavelet_coeffs_scale_{}.png".format(scale_idx + 1)
            ),
    ...
    logger.info(
        "{} -> Saved decomposition components and visualizations".format(header)
    )
