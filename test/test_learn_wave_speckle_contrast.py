...
def test():
    ...
    kernel = (
        torch.tensor([[[[0, -1, 0], [-1, 4, -1], [0, -1, 0]]]], dtype=torch.float32) / 4
    )
    kernel_size = 7
    step_size = (3, 3)
    phase_gradient_calculator = phase_gradient(kernel=kernel, loss=nn.MSELoss())

    speckle_contrast_calculator = speckle_contrast(
        kernel_size=kernel_size, step_size=step_size, loss=nn.MSELoss()
    )
