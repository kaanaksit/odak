...
def test(
    ...
    weights_filename = "{}/multi_layer_perceptron_model_weights.pt".format(
        output_directory
    )
    learning_rate = 1e-4
    no_epochs = 2  # 100000
    ...
    scheduler = torch.optim.lr_scheduler.PolynomialLR(
        optimizer, total_iters=no_epochs, power=1.0
    )
    ...
    test_resolution = [
        original_resolution[0] * test_resolution_scale,
        original_resolution[1] * test_resolution_scale,
        3,
    ]
    ...
    try:
        for epoch_id in epochs:
            ...
            description = "train loss: {:.8f}, learning rate: {:.8f}".format(
                train_loss, current_learning_rate
            )
