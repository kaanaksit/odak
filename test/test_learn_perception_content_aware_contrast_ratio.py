...
def test_content_aware_contrast_ratio_different_window_sizes():
    ...
    for window_size in [5, 9, 15, 21]:
        result = content_aware_contrast_ratio(image, window_size=window_size, step=3)
...
def test_content_aware_contrast_ratio_different_steps():
    ...
    for step in [1, 3, 5]:
        result = content_aware_contrast_ratio(image, window_size=15, step=step)
        assert result["contrast_map"].shape == (128, 128)
...
def test_content_aware_contrast_ratio_gradient_flow():
    ...
    content_aware_contrast_ratio(image, window_size=15, step=3)
...
def test_content_aware_contrast_ratio_error_invalid_percentile():
    ...
    # Percentile <= 0
    with pytest.raises(ValueError):
        content_aware_contrast_ratio(
            image, window_size=15, step=3, pooling_percentile=0.0
        )

    # Percentile > 100
    with pytest.raises(ValueError):
        content_aware_contrast_ratio(
            image, window_size=15, step=3, pooling_percentile=105.0
        )
