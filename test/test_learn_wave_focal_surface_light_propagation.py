...
def test(output_directory="test_output"):
    ...
    focal_surface = odak.learn.tools.load_image(
        focal_surface_filename, normalizeby=255.0, torch_style=True
    ).to(device)
    distances = (
        torch.linspace(-volume_depth / 2.0, volume_depth / 2.0, number_of_planes)
        + location_offset
    )
    ...
    focal_surface_light_propagation_model = (
        odak.learn.wave.focal_surface_light_propagation(device=device)
    )
    focal_surface_light_propagation_model.load_weights(
        weight_filename=weight_filename, key_mapping_filename=key_mapping_filename
    )
