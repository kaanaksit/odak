...
def test(device=torch.device("cpu"), output_directory="test_output"):
    ...
    target = load_image(
        "test/data/sample_rgb.png", normalizeby=256, torch_style=True
    ).to(device)
    target = rgb_to_linear_rgb(target).squeeze(0)

    depth = load_image(
        "test/data/sample_depthmap.png", normalizeby=256, torch_style=True
    ).to(device)
    depth = torch.mean(depth, dim=0).to(
        device
    )  # Ensure the depthmap has the shape of [w x h]
    ...
    pixel_pitch = 8.2e-06
    apertures = [1.0, 1.0, 1.0]

    targets, masks = slice_rgbd_targets(target, depth, depth_plane_positions)
    distances = (
        depth_plane_positions * 0.001
    )  # Multiply with some multiplier to control the blurriness

    for idx, mask in enumerate(masks):
        save_image(join(output_directory, f"mask_{idx}.png"), mask, cmin=0, cmax=1)

    for idx, target in enumerate(targets):
        save_image(join(output_directory, f"target_{idx}.png"), target, cmin=0, cmax=1)
