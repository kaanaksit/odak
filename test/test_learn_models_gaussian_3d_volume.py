...
def visualize(
    ...
    diagram.set_axis_limits(
        x_limits=[-1, 1], y_limits=[-1, 1], z_limits=[-1, 1], column=1
    )
    diagram.set_axis_limits(
        x_limits=[-1, 1], y_limits=[-1, 1], z_limits=[-1, 1], column=2
    )
    diagram.set_axis_limits(
        x_limits=[-1, 1], y_limits=[-1, 1], z_limits=[-1, 1], column=3
    )
...
def main(
    ...
    loss_weights={
        ...
        "alpha": {"smaller": 1.0e-2, "larger": 0.0, "threshold": [1e-2, 1.0]},
        "scale": {"smaller": 1.0e-2, "larger": 0.0, "threshold": [1e-3, 1.0]},
        "angle": 0e-0,
        "center": 1e-2,
        "utilization": {
            "l2": 1.0e-2,
    ...
    points, ground_truth = odak.learn.tools.load_voxelized_PLY(
        ply_filename=ply_filename, voxel_size=ply_voxel_size, device=device
    )

    model = odak.learn.models.gaussian_3d_volume(
        number_of_elements=number_of_elements
    ).to(device)
    model.load_weights(weights_filename=weights_filename, device=device)
