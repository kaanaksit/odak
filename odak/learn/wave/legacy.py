...
class multiplane_hologram_optimizer:
    ...
    def __init__(
        ...
        logger.warning(
            "odak.learn.wave.multiplane_hologram_optimizer module will be removed in the next releases. Please consider using odak.learn.wave.multi_color_hologram_optimizer instead."
        )
        self.device = device
        ...
        self.scene_center = self.image_spacing * (self.number_of_planes - 1) / 2.0
    ...
    def init_amplitude(self, amplitude_initial):
        ...
        if isinstance(self.amplitude, type(None)):
            self.amplitude = torch.ones(
                self.slm_resolution[0], self.slm_resolution[1], requires_grad=False
            ).to(self.device)
    ...
    def init_phase(self, phase_initial):
        ...
        if isinstance(self.phase, type(None)):
            self.phase = (
                torch.rand(self.slm_resolution[0], self.slm_resolution[1])
                .detach()
                .to(self.device)
                .requires_grad_()
            )
    ...
    def init_mask(self):
        ...
        self.mask[
            int(self.slm_resolution[0] * self.mask_limits[0]) : int(
                self.slm_resolution[0] * self.mask_limits[1]
            ),
            int(self.slm_resolution[1] * self.mask_limits[2]) : int(
                self.slm_resolution[1] * self.mask_limits[3]
            ),
        ] = 1
    ...
    def optimize(self):
        ...
        reconstruction_intensities = self.reconstruct(
            hologram_amplitude, hologram_phase
        )
        return (
            hologram_phase.detach().clone(),
            hologram_amplitude.detach().clone(),
            reconstruction_intensities.detach().clone(),
        )
    ...
    def reconstruct(self, hologram_amplitude, hologram_phase):
        ...
        for plane_id in range(self.number_of_planes):
            ...
            reconstruction_intensities[plane_id] = (
                calculate_amplitude(reconstruction) ** 2
            )
