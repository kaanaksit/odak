...
def generate_bspline_basis_2d(
    num_control_points, resolution, device=torch.device("cpu")
):
    """
    ...
    control_grid = torch.stack(
        torch.meshgrid(control_x, control_y, indexing="ij"), dim=-1
    )
...
def decompose_bspline_2d(image, num_control_points, regularization=1e-6):
    ...
    basis_matrix, control_grid = generate_bspline_basis_2d(
        num_control_points, [H, W], device
    )
...
def decompose_wavelet_like(image, n_scales=4):
    ...
    for scale in range(n_scales):
        ...
        coeffs, basis, _ = decompose_bspline_2d(
            current, [cp_size, cp_size], regularization=1e-8
        )
