...
def extract_local_patches(
    image: torch.Tensor, patch_size: int = 4, stride: int = 2
...
def compute_local_contrast_similarity(
    patch_x: torch.Tensor, patch_y: torch.Tensor, epsilon: float = 1e-8
) -> torch.Tensor:
    """
    ...
    if patch_x.shape != patch_y.shape:
        raise ValueError(
            f"patch shapes must match, " f"got {patch_x.shape} vs {patch_y.shape}"
        )
...
def local_contrast_loss(
    ...
    for b in range(B):
        sim = compute_local_contrast_similarity(
            patches_original[b], patches_simulated[b], epsilon
        )
        ...
        for b in range(B):
            for c in range(C):
                ...
                blurred_channel = blur_gaussian(
                    channel, kernel_length=kernel_length, nsigma=nsigma, padding="same"
                )
...
def cvd_loss(
    ...
    lc_loss = local_contrast_loss(
        image, simulated_image, patch_size=lc_patch_size, stride=lc_stride
    )

    ci_loss = color_information_loss(
        image, simulated_image, kernel_size=ci_kernel_size, sigma=ci_sigma
    )
...
def _validate_image_pair(image: torch.Tensor, simulated_image: torch.Tensor) -> None:
    """
    ...
    if not isinstance(simulated_image, torch.Tensor):
        raise TypeError(
            f"simulated_image must be torch.Tensor, got {type(simulated_image)}"
        )
    ...
    if simulated_image.dim() != 4:
        raise ValueError(
            f"simulated_image must be 4D [B, C, H, W], got {simulated_image.dim()}D"
        )
