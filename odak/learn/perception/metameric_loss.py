...
class MetamericLoss():
    """
    Class to calculate metameric loss between input and target images.
    =================================================================
    Parameters:
    ...
    use_l2_foveal_loss: If true, for all the pixels that have pooling size 1 pixel in the 
    ...
    fovea_weight: A weight to apply to the foveal region if use_l2_foveal_loss is set to True.
    use_radial_weight: If True, will apply a radial weighting when calculating the difference between
        the source and target stats maps. This weights stats closer to the fovea more than those
        further away.
    use_fullres_l0: If true, stats for the lowpass residual are replaced with blurred versions
    ...
    def __init__(self, device=torch.device('cpu'), alpha=0.08, real_image_width=0.2, \
        ...
        if self.use_fullres_l0 and self.use_l2_foveal_loss:
            raise Exception("Can't use use_fullres_l0 and use_l2_foveal_loss options together in MetamericLoss!")
    ...
    def calc_statsmaps(self, image, gaze=None, alpha=0.01, real_image_width=0.3,\
        ...
        def find_stats(image_pyr_level, blur):
            image_means  = blur.blur(image_pyr_level, alpha, real_image_width, real_viewing_distance, centre=gaze, mode=mode)
            image_meansq = blur.blur(image_pyr_level*image_pyr_level, alpha, real_image_width, real_viewing_distance, centre=gaze, mode=mode)
        ...
        output_stats  = []
        image_pyramid = self.pyramid_maker.construct_pyramid(image, self.n_pyramid_levels)
        ...
        if self.use_l2_foveal_loss:
            ...
            for i in range(self.fovea_mask.size(1)):
                self.fovea_mask[0, i, ...] = 1.0 - (self.blurs[0].lod_map / torch.max(self.blurs[0].lod_map))
        ...
        for l in range(0,len(image_pyramid)-1):
            for o in range(len(image_pyramid[l]['b'])):
                ...
                if self.use_l2_foveal_loss:
                    ...
                    output_stats.append(variances)
            if self.use_l2_foveal_loss:
                periphery_mask = torch.nn.functional.interpolate(periphery_mask, scale_factor=0.5, mode="area", recompute_scale_factor=False)
        
        if self.use_l2_foveal_loss:
            output_stats.append(image_pyramid[-1]["l"] * periphery_mask)
        elif self.use_fullres_l0:
            output_stats.append(self.blurs[0].blur(image, alpha, real_image_width, real_viewing_distance, gaze, mode))
        else:
    ...
    def visualise_loss_map(self, image_stats):
        ...
        for i in range(len(image_stats)):
            ...
            stat_mse_map = torch.nn.functional.interpolate(stat_mse_map, size=loss_map.size(), mode="bilinear", align_corners=False, recompute_scale_factor=False)
    ...
    def __call__(self, image, target, gaze=[0.5,0.5], image_colorspace="RGB", visualise_loss=False):
        """
        gaze: The gaze location in normalised coordinates (in range [0,1]) relative to the top left of the image.
        image_colorspace: The current colorspace of your image and target. Ignored if input does not have 3 channels.
            accepted values: RGB, YCrCb.
        visualise_loss: Shows a heatmap indicating which parts of the image contributed most to the loss. 
        """
        #TODO if input is RGB, convert to YCrCb.
        if image.size(1) != target.size(1):
            raise Exception("MetamericLoss ERROR: Input and target must have same number of channels.")
        ...
        if type(target) == type(self.target):
            ...
            if self.use_l2_foveal_loss:
                peripheral_loss = self.metameric_loss_stats(image_stats, self.target_stats, gaze)
