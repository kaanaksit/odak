...
__title__   = 'Odak with CUDA support'
...
class raytracing():
    ...
    def create_ray(self,x0y0z0,abg):
        """
    ...
    def create_ray_from_two_points(self,x0y0z0,x1y1z1):
        """
    ...
    def multiply_two_vectors(self,vector1,vector2):
        """
        ...
        ray          : ndarray
                       Array that contains starting points and cosines of a created ray.
        """
        ##### Cross isn't implemented for gpu, yet. ####
        ##### Cross isn't implemented for gpu, yet. ####
        ##### Cross isn't implemented for gpu, yet. ####
        angle  = np_cpu.cross(np.asnumpy(vector1[1]),np.asnumpy(vector2[1]))
    ...
    def find_intersection_w_surface(self,ray,points):
        """
        ...
        vector0,s            = self.create_ray_from_two_points(point0,point1)
        vector1,s            = self.create_ray_from_two_points(point1,point2)
        vector2,s            = self.create_ray_from_two_points(point0,point2)
        normal               = self.multiply_two_vectors(vector0,vector2)
        f                    = point0-ray[0]
        n                    = normal[1].copy()
    ...
    def reflect(self,input_ray,normal):
        """
    ...
    def define_plane(self,point,angles=[0.,0.,0.]):
        """
        ...
        for i in range(0,plane.shape[0]):
            plane[i],_,_,_  = self.rotate_point(plane[i],angles=angles)
    ...
    def rotmatx(self,angle):
        ...
        angle = self.angle_to_radians(angle)
        rotx  = np.array([
                          [1.,            0.  ,           0.],
    ...
    def rotate_point(self,point,angles=[0,0,0],mode='XYZ'):
        """
        ...
        rotz         : ndarray
                       Rotation matrix along Z axis.
        """
        rotx   = self.rotmatx(angles[0])
        roty   = self.rotmaty(angles[1])
        rotz   = self.rotmatz(angles[2])
        if mode == 'XYZ':
